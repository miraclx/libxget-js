  ...
  return pipeAll
    .call(this, {
      input: data.stream
        ...
        .once('error', err => this.destroy(((err.index = index), err))),
      data,
      pipeStack,
      store,
    })
...
function xonstructor(url, opts) {
  ...
  function handleLoad(loadData) {
    ...
    if (store.hash.hasher) core = core.pipe(store.hash.hasher);
    core
      .on('data', data => {
  ...
  this._destroy = (err, cb) => {
    const doDestroy = () => (store.chunkStack.forEach(data => data.stream.destroy()), cb(err));
