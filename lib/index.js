...
async function getContentStruct(url, timeout) {
  ...
  const getData = extractData(
    await new Promise((res, rej) =>
      request
        .get(url, {timeout, headers: {Range: 'bytes=0-0'}})
        .on('response', resp => (resp.req.abort(), res(resp)))
        .on('error', rej),
    ),
  );
  if (
    getData.statusCode === 416 || // Requested range not satisfiable
...
async function xHeadData(store, opts) {
  const {headers, statusCode, statusMessage, unHEADRangeable} = await getContentStruct(store.url, store.timeout);
