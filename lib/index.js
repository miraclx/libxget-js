...
async function getContentStruct(store) {
  ...
  let headData = {headers: {}, status: 0, statusText: ''};
  try {
    headData = extractData(await axiosInstance.head(store.url));
  } catch (err) {
    if (err.response && err.response.status !== 403) throw err;
  }
  if (headData.headers['content-length']) return headData;
  const getData = extractData(await axiosInstance.get(store.url, {headers: {Range: '0-0'}}));
  if (Object.keys(headData.headers).length === 0) headData.headers = getData.headers;
  if (!headData.status) headData.status = getData.status;
  if (!headData.statusText) headData.statusText = getData.statusText;
  if (
...
async function xHeadData(store, opts) {
  ...
  const acceptsRanges = !unHEADRangeable && headers['accept-ranges'] === 'bytes';
  const offset = await store.headHandler({headers, acceptsRanges});
  if (typeof offset === 'number') store.start = offset;
  ...
  if (!Number.isFinite(store.size)) {
    ...
    store.size = store.totalSize - store.start;
  }
  if (acceptsRanges && Number.isFinite(store.size)) store.chunkable = true;
...
function xonstructor(url, opts) {
  ...
  this.start = () => !store.auto && (initHead.call(this, store, opts), true);
