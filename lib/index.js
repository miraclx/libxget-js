...
function xonstructor(url, opts) {
  ...
  function handleLoad() {
    store.core
      .add(...store.chunkStack.map((data, index) => handleChunk.call(this, {data, index, store})))
      .on('data', data => {
  ...
  xHeadData(store, opts)
    ...
    .catch(err => {
      err.stack = err.stack.replace(
        /(.+?:\s)[^]+?((\s*)\s{2}at)/m,
        `$1An error occurred while ${store.loaded ? 'processing information from' : 'trying to get information for'} the URL$3${
          err.message
        }$2`,
      );
      this.emit('error', err);
