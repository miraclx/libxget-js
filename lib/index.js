...
function buildChunks(url, spec, opts, {retries, start}) {
  ...
  chunkAxiosInstance.interceptors.response.use(({data, request}) => {
    let erred = false;
    let timedOut = false;
    const throwErr = err => () => !erred && (data.emit('error', err), (erred = true));
    data.once('aborted', throwErr(new XgetException(timedOut ? 'Connection Timed Out' : 'Connection Terminated')));
    request.once('timeout', () => (timedOut = true));
    return data;
  });
...
function pipeAll({input, data, store}) {
  ...
  const sxTream = Object.entries(store.pipeStack).reduce((xStream, [tag, fn], _, arr) => {
    ...
    try {
      ...
      er[store.ErrTag] = tag;
      er[store.ErrSource] = 'xget:use';
      xStream.destroy();
      er.stack = er.stack.replace(
        /(.+?:\s)[^]+?((\s*)\s{2}at)/m,
        `$1An error occurred while creating a duplex transformer from the use middleware tagged [${tag}] $3${er.message}$2`,
      );
      this.destroy(er);
      caughtErr = true;
      arr.splice(1);
...
function xonstructor(url, opts) {
    ...
    .catch(err => {
      err.stack = err.stack.replace(
        ...
        `$1An error occurred while ${
          err[store.ErrSource]
            ? `processing the data through the with middleware tagged [${err[store.ErrTag]}]`
  ...
  this._destroy = (err, cb) => {
    const doDestroy = () => (
      store.chunkStack.forEach(data => {
        if (data.errHandler) data.stream.removeListener('error', data.errHandler);
        data.stream.destroy();
      }),
      cb(err)
    );
  ...
  this.getErrContext = raw => ({
    raw,
    tag: raw[store.ErrTag],
    source: raw[store.ErrSource],
  });
}
