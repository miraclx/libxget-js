...
function xonstructor(url, opts) {
  ...
  if (XGET_CHECK_VAR(opts.cache, 'opts.cache', StreamCache, true)) (store.cache = opts.cache), delete opts.cache;
  if (XGET_CHECK_VAR(opts.useCache, 'opts.useCache', 'boolean', true)) (store.useCache = opts.useCache), delete opts.useCache;
  ...
  if (store.hash.algorithm) {
    ...
    store.hash.hasher = new crypto.createHash(store.hash.algorithm);
  }
