...
function test() {
  ...
  const OUT1 = `${filename}.test1${ext}`;
  const OUT2 = `${filename}.test2${ext}`;
  const OUT3 = `${filename}.test3${ext}`;
  const OUT4 = `${filename}.test4${ext}`;

  const cache = new StreamCache({size: SIZE});
  ...
  const barGen = ProgressBar.stream(fs.statSync(FILE).size * 4, ProgressBar.slotsByCount(4), {
    ...
    template: [
      ...
      ' |:{bar:complete}| [:3{slot:percentage}%] (:{slot:eta}) [:{speed}] [:{slot:size}/:{slot:size:total}]',
  ...
  }).on('complete', () => barGen.end('[+] Test Complete\n'));
