...
import {randomBytes} from './streamUtils.js';
...
async function test() {
  ...
  const {default: xbytes} = await import('xbytes');
  const {default: ProgressBar} = await import('xprogress');
  ...
  if (!['1', '2', '3', '4'].includes(TESTCODE)) throw new Error(`Invalid testCode [${TESTCODE}]. Expected: 1..4`);
