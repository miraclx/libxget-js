...
class StreamCache {
  ...
  #allocOn = function allocOn(stream, stack, chunk, callback) {
    // ||| pending ||  full | reading ||| | action ||
    // |||---------||-------|---------||| |--------||
    // |||    true ||     ~ |       ~ |||=|  alloc ||
    // |||   false ||  true |    true |||=|   push ||
    // |||   false ||  true |   false |||=|  alloc ||
    // |||   false || false |   false |||=|  alloc ||
    // |||---------||-------|---------||| |--------||
    // * full    : the cache is full
    ...
    if (stack.pendingWrites || !(this.#isFull() && stack.pendingReads)) {
      stack.pendingWrites += 1;
      this.#store.allocBuffer.push({stream, stack, chunk, callback});
      this.dispatchAllocs();
    } else {
      // reason for this:
      //  • stream may have active readers and the cache may be full
      //  • with chunks from streams other than this one theres no point
      //  • having this one wait till other streams read their chunks
      //  • in fact, other streams could depend on this one, maybe a sequential stream
      //  • Basically, this stream has zero pending chunks either in the cache or allocBuffer
      // solution: push new items directly
      stream.push(chunk);
      if (stack.pendingReads) stack.pendingReads -= 1;
      callback();
    }
  };
